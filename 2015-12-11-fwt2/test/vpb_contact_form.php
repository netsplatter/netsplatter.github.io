...
{
	//assigment
	$to_email          = "drunkraven@yandex.ru"; // Replace this email field with your email address or your company email address
	...
	$phone_number      = isset($_POST['phone']) ? trim(strip_tags($_POST['phone'])) : '';
	...
	mail($to_email, $email_subject, $vpb_message_body, $headers);
	...
	if($from_fullname == "")
	{
		echo '<br clear="all"><div class="vpb_info" align="left">Please enter your fullname in the required field to proceed. Thanks.</div>';
	}
	elseif($from_email == "")
	{
		echo '<br clear="all"><div class="vpb_info" align="left">Please enter your email address in the required email field to proceed. Thanks.</div>';
		...
		else
		{
			 if(@mail($to_email, $email_subject, $vpb_message_body, $headers))
			 {
				//Displays the success message when email message is sent
				  echo "<br clear='all'><div align='left' class='vpb_success'>Congrats ".$from_fullname.", your email message has been sent successfully!<br>We will get back to you as soon as possible. Thanks.</div>";
			 } 
			 else 
			 {
